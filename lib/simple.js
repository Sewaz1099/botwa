...
`.trim()
    ...
    async acceptInvite(jid) {
     let link = jid.split('com/')[1]
        let response = await this.query({ json: ['action', 'invite', link], expect200: true })
        return response
    }
    ...
    async revokeInvite(jid) {
        let response = await this.query({ json: ['action', 'inviteReset', jid], expect200: true })
        return response
    }
