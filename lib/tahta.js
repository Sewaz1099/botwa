...
function ht(text = '') {
  return new Promise((resolve, reject) => {
    ...
    //let font = path.join(_font, pickRandom(fs.readdirSync(_font)))
    let font = path.join(_font, 'AlbertusNovaBlack.ttf')
    let w = 1024
    let h = w
    let s = w + 'x' + h
...
function ctahta(text = '', text2 = '', text3 = '') {
  return new Promise((resolve, reject) => {
    let img = path.join(aesthetic, pickRandom(fs.readdirSync(aesthetic)))
    let font = path.join(_font, pickRandom(fs.readdirSync(_font)))
    //let font = path.join(_font, '212BabyGirl.otf')
    let w = 1024
    let h = w
    let s = w + 'x' + h
    let xF = `(${noise('X', 2, w, 1)}+${noise('Y', 1, h, 1)})/2+128`
